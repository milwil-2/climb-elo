...
async def v2_live_projections_json(event_id: int, gender: str = "M"):
    """Return current projection data for a live event as JSON (v2 surface).

    Mirrors ``/live/{event_id}/projections.json`` from routes.py but is
    mounted under /v2/.  The SSE consumer in templates_v2/live.html calls
    this endpoint so the projection panel can refresh with a small JSON
    payload (~1-5 KB) instead of re-fetching the full HTML page (~50-100 KB).
