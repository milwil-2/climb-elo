#!/usr/bin/env python3
"""One-shot migration: add ``engine_version`` to the EventForecast unique key.

Issue #124. The original ``uq_event_forecast_event_gender_athlete_backfill``
constraint keyed on ``(event_id, gender, athlete_id, is_backfill)``; bumps to
``engine_version_tag()`` overwrote prior-version snapshots. The new constraint
``uq_event_forecast_event_gender_athlete_backfill_version`` keys on
``(event_id, gender, athlete_id, is_backfill, engine_version)`` so re-snapshots
at a new engine version insert a fresh row alongside the prior one.
...
Usage::
    ...
    uv run python scripts/migrate_forecast_engine_version_constraint.py
"""
...
_TABLE = "event_forecasts"
_OLD_COLS = {"event_id", "gender", "athlete_id", "is_backfill"}
_NEW_COLS = {"event_id", "gender", "athlete_id", "is_backfill", "engine_version"}
_NEW_NAME = "uq_event_forecast_event_gender_athlete_backfill_version"
    ...
    with engine.begin() as conn:
        ...
        conn.execute(
            text(
                ...
                "UNIQUE (event_id, gender, athlete_id, is_backfill, engine_version)"
            )
        )
