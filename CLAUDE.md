...
Glicko-2 RD–weighted ELO with 538-style gap-conditioned margin-of-victory. K-factor table tiered by EventTier × RoundType (Olympics Final = 48 unchanged for lack of source-DB data; **World Cup Final = 12, WCh Final = 15, Continental Final = 6** post-#80 regrid on 2026-05-27 — see `docs/K_REGRID_REPORT.md`). Per-round effective K is `K_base · g(φ_opponent) · margin_mult`, so opponents with high uncertainty (cold start, post-sabbatical) contribute less. The legacy 2× provisional-K cliff is gone — cold start is now driven by Glicko-2's large initial φ (σ=350) and the closed-form φ shrinkage. σ decays during inactivity via Glicko-2's Wiener-process formula (**`σ_inactivity=25.0` post-#89**, 30-day grace), clamped to [50, 350]. MARGIN_CAP=1.5 remains as a backstop on the base multiplier; the new gap-conditioning (#53) damps favourite-side bonuses asymmetrically — `mult = base · 2.2/(max(Δμ,0)/400 + 2.2)` — so an elite crushing a junior earns less than a peer crushing a peer, while upsets keep the full bonus. Score normalization unchanged: Lead `"34+"`→34.5, `"TOP"`→999.0; Boulder `tops*1000 + zones*100 - top_att*10 - zone_att` (max_gap=1000) for pre-2025 events, decimal pass-through for 2025+ events; Speed in seconds (max_gap=2.0).
...
**Tournament Participation Bonus (Issue #90 — Gap 1 from #88).** A tier-weighted, **zero-sum** μ credit applied per event on top of the pairwise round updates. After all rounds of an event have been processed, the FINAL-round results are passed to `compute_tournament_participation_bonus()` (in `engine/elo.py`): top-K finishers receive a gross μ credit from `EloConfig.tpb_table` and the **total** credit is debited uniformly across **every** non-DNS participant, so the event sum is exactly zero. The starting tier table (file follow-ups welcome): Olympics top-8 (peak +30μ for #1, linear decay to 0 at rank 8); World Championship top-8 (+20μ peak); World Cup top-6 (+12μ peak); Continental top-4 (+5μ peak). TPB is persisted as a synthetic `RatingHistory` row with `kind='tpb'` whose `round_id` points at the event's final round — the breakdown page renders it as its own section under the pairwise contests. Idempotent via the `(athlete_id, round_id, kind)` unique constraint; safe to re-run backfill. Full design: `docs/PLAN_FIELD_STRENGTH_AND_ACTIVITY.md`. **Activating TPB in production requires a re-backfill** (`gh workflow run scrape-supabase.yml`).
